...
<div class="mx-auto flex h-dvh w-full max-w-5xl flex-col gap-4 overflow-hidden p-4 pb-6 sm:p-6">
	<header class="press-card border bg-card p-4 sm:p-5">
		<div class="flex items-start justify-between gap-4">
			<div>
				<p class="text-xs font-bold tracking-[0.32em] text-muted-foreground uppercase">Article ledger</p>
				<h1 class="mt-2 font-heading text-5xl leading-none font-black tracking-[-0.06em] sm:text-7xl">Clips</h1>
			</div>
	...
	<Tabs.Root bind:value={filter} class="min-h-0 flex-1 overflow-hidden rounded-3xl border bg-[var(--press-glass)] p-2 backdrop-blur-xl">
		<Tabs.List variant="line" class="sticky top-0 z-10 mb-2 w-full justify-start overflow-x-auto bg-transparent p-2">
		...
		<ul class="h-[calc(100%-3.75rem)] overflow-y-auto p-2 sm:grid sm:auto-rows-min sm:grid-cols-2 sm:gap-3">
			{#each filtered as article (article.id)}
				<li style={`--accent:${SECTION_ACCENT[article.section]}`}>
					{#if article.state === 'hidden'}
						<div class="mb-3 grid grid-cols-[0.6rem_1fr_auto] gap-3 rounded-2xl border bg-card/70 p-3 opacity-75">
							...
							<div class="min-w-0">
								...
								<p class="mt-1 text-[0.68rem] font-bold tracking-[0.18em] text-muted-foreground uppercase">
									<span class="block truncate">{article.source} · {article.section} · {LANGUAGE_LABEL[articleLanguage(article)]}</span>
					...
					{:else}
						...
						>
							<span class="rounded-full bg-[var(--accent)]" aria-hidden="true"></span>
							<span class="min-w-0">
								<span class={cn('block truncate font-semibold', article.state === 'read' && 'text-muted-foreground')}>
									{article.headline}
								</span>
								<span class="mt-1 block text-[0.68rem] font-bold tracking-[0.18em] text-muted-foreground uppercase">
									<span class="block truncate">
										{article.source} · <span class="text-[var(--accent)]">{article.section}</span> · {LANGUAGE_LABEL[articleLanguage(article)]}
									</span>
									<span class="block text-foreground/70">{stamp(article.publishedAt)}</span>
								</span>
							</span>
			...
			{:else}
				<li class="rounded-2xl border border-dashed p-8 text-center text-muted-foreground sm:col-span-2">
